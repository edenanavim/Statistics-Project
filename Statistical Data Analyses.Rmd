...
# mdl1_sib <- glm(is_atopic_first_year ~ SES_group + gender + type_of_pregnancy +
...
# mdl2_sib <- glm(is_atopic_second_year ~ SES_group + gender + 
#              mother_Atopic_diseases + father_Atopic_diseases + sibling_Atopic_diseases + 
#              mother_year_birth + mother_education + number_of_children + 
#              type_of_pregnancy  + vaccine_12mo  + nursing_setup_at_12mo + covid_time, data=df_mdl, family='binomial')
...
# df_mdl$mdl1_sib <- mdl1_sib$fitted.values
df_mdl$mdl2 <- predict(mdl2)
# df_mdl$mdl2_sib <- mdl2_sib$fitted.values
...
# gam1_sib <- gam(is_atopic_first_year~ SES_group + gender + type_of_pregnancy + 
#              mother_Atopic_diseases + father_Atopic_diseases + sibling_Atopic_diseases+
#              mother_year_birth + mother_education + number_of_children + 
#              s(DOB_count, bs='cr') ,data=df_mdl, family=binomial)
...
# gam2_sib <- gam(is_atopic_second_year~ SES_group + gender + 
#              mother_Atopic_diseases + father_Atopic_diseases + sibling_Atopic_diseases +
#              mother_year_birth + mother_education + number_of_children + 
#              type_of_pregnancy  + vaccine_12mo  + nursing_setup_at_12mo + s(DOB_count, bs='cr') ,data=df_mdl, family=binomial)
df_mdl$gam1 <- predict(gam1)
# df_mdl$gam1_sib <- gam1_sib
df_mdl$gam2 <- predict(gam2)
# df_mdl$gam2_sib <- gam2_sib
```
...
# First year - No Sibilings
ggplot(df_mdl, aes(DOB, gam1)) + geom_smooth() + geom_line(aes(DOB, mdl1_avg, col = covid_time)) + 
  design + labs(title = 'Atopic disease risk for babies up to one year old by DOB', x = 'Date of Birth', y = 'Atopic disease probability') +
  scale_color_manual(values=viridis(4), name = "Born at", labels =c('Up to March19','March19 - March20','March20 - Feb21', 'From Feb21' ))+ ylim(0.5,1)
...
נקודות שמיכה אמר בפגישה :

צריך להסתגל על הגרף של האוד רשיו ולא על הגרף של ההסבתרות כי הוא תלוי גם במשתנים האחרים 
