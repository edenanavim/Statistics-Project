...
df_mdl$mdl1 <- mdl1$fitted.values
df_mdl$mdl1_sib <- mdl1_sib$fitted.values
df_mdl$mdl2 <- mdl2$fitted.values
df_mdl$mdl2_sib <- mdl2_sib$fitted.values

df_group <- df_mdl %>% group_by(covid_time) %>% summarise(mdl1_avg = mean(mdl1, na.rm=T), mdl1_sib_avg = mean(mdl1_sib, na.rm=T), 
                                                          mdl2_avg = mean(mdl2, na.rm=T), mdl2_sib_avg = mean(mdl2_sib, na.rm=T))
...
gam2_sib <- gam(is_atopic_second_year~ SES_group + gender + 
             ...
             mother_year_birth + mother_education + number_of_children + 
             type_of_pregnancy  + vaccine_12mo  + nursing_setup_at_12mo + s(DOB_count, bs='cr') ,data=df_mdl, family=binomial)
df_mdl$gam1 <- gam1$fitted.values
df_mdl$gam1_sib <- gam1_sib$fitted.values
df_mdl$gam2 <- gam2$fitted.values
df_mdl$gam2_sib <- gam2_sib$fitted.values
```
