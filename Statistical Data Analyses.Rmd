...
agg_table <- df %>% group_by(covid_time) %>% summarise(n = n(), first_year = sum(is_atopic_first_year), second_year = sum(is_atopic_second_year) )
agg_table %>% mutate(first_perc = round(100*first_year/ n), second = round(100*second_year/n))
```
...
mdl2 <- glm(is_atopic_second_year ~ SES_group + gender + 
             ...
             type_of_pregnancy  + vaccine_12mo  + nursing_setup_at_12mo + covid_time+ 
             is_atopic_first_year, data=df_mdl, family='binomial')
...
gam2 <- gam(is_atopic_second_year ~ SES_group + gender + 
             mother_Atopic_diseases + father_Atopic_diseases +
             mother_year_birth + mother_education + number_of_children + 
             type_of_pregnancy  + vaccine_12mo  + nursing_setup_at_12mo + s(DOB_count, bs='cr') +
             is_atopic_first_year, data=df_mdl, family=binomial)
