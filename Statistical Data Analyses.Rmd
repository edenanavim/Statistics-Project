...
# Analyze after meeting with micha : 
```{r}
explanatory_kpi = c('Jew/Arab', 'gender', 'Allergy_total','vaccine_12mo', 'prevention_therapy',
                ...
                'family_Atopic_diseases_sum', 'mother_Atopic_diseases_sum', 'father_Atopic_diseases_sum', 'sibling_Atopic_diseases_sum',
                'sibling_asthma_sum', 'sibling_AD_sum', 'sibling_Allergic_rhinitis_sum', 'sibling_FA_sum',
                'type_of_pregnancy', 'nursing_setup_at_12mo', 'covid_time')

mdl1 <- glm(is_atopic_first_year~ `Jew/Arab` + gender + Allergy_total + prevention_therapy + 
             family_Atopic_diseases + mother_Atopic_diseases + father_Atopic_diseases + sibling_Atopic_diseases + 
             mother_year_birth + DOB + mother_education + SES + number_of_children + preganancy_number + 
             type_of_pregnancy + covid_time, data = df, family = "binomial")
summary(mdl1)

mdl2 <- glm(is_atopic_second_year~ `Jew/Arab` + gender + Allergy_total + vaccine_12mo + prevention_therapy + 
             family_Atopic_diseases + mother_Atopic_diseases + father_Atopic_diseases + sibling_Atopic_diseases + 
             mother_year_birth + DOB + mother_education + SES + number_of_children + preganancy_number + 
             number_of_ABX_first_year + number_inhalation + type_of_pregnancy + nursing_setup_at_12mo + covid_time, data = df, family = "binomial")
summary(mdl2)

mod_gam1 = gam(is_atopic_first_year ~  gender + Allergy_total + prevention_therapy + 
             family_Atopic_diseases + mother_Atopic_diseases + father_Atopic_diseases + sibling_Atopic_diseases + 
             mother_year_birth + DOB + mother_education + SES + number_of_children + preganancy_number + 
             type_of_pregnancy + s(date_of_birth, bs = "cr"), data = df)
summary(mod_gam1)
plot(mod_gam1)

df$date_of_birth = as.numeric(as.Date('2018-01-14'))/ as.numeric(as.Date(df$DOB)) 



```

# Logistic regression after finding SRM 
```{r}
SRM_kpi = c('vaccine_12mo ', 'number_of_ABX_first_year', 'type_of_pregnancy', 'nursing_setup_at_12mo')


mdl1 <- glm(is_atopic_first_year~  type_of_pregnancy + covid_time, data = df, family = "binomial")
summary(mdl1)

mdl2 <- glm(is_atopic_second_year~ vaccine_12mo + number_of_ABX_first_year  + type_of_pregnancy+ nursing_setup_at_12mo + is_atopic_first_year + covid_time, data = df, family = "binomial")
summary(mdl2)

# data = df %>% group_by(covid_time) %>% summarise(is_atopic_first_year = sum(is_atopic_first_year), is_atopic_second_year = sum(is_atopic_second_year, na.rm = T), n = n_distinct(record_id))
# data$prop1 = data$is_atopic_first_year / data$n
# data$prop2 = data$is_atopic_second_year / data$n
# 
# data
