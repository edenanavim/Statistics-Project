...
atopic_longer_count <- df %>% select(c(no_is_atopic, covid_time,)) %>% drop_na() %>% group_by(covid_time) %>% summarise_if(is.numeric, mean) %>% pivot_longer(cols  =no_is_atopic, names_to = "chronic_disease", values_to = "mean") 
...
is_atopic_df <- df %>% select(c(is_atopic, covid_time,)) %>% drop_na() %>% group_by(covid_time) %>% summarise_if(is.numeric, mean) %>% pivot_longer(cols = is_atopic, names_to = "chronic_disease", values_to = "mean") 

is_atopic_longer <- is_atopic_df %>% mutate(Born_at = ifelse(covid_time == 1, 'Up to March19', ifelse(covid_time == 2, 'March19 - March20', 'March20 - Feb21')))
...
new_df <- df %>% select(c(atopic_binari_exp, covid_time,)) %>% drop_na() %>% group_by(covid_time) %>% summarise_if(is.numeric, mean) %>% pivot_longer(cols = atopic_binari_exp, names_to = "chronic_disease", values_to = "mean") 
new_df_longer <- new_df %>% mutate(Born_at = ifelse(covid_time == 1, 'Up to March19', ifelse(covid_time == 2, 'March19 - March20', 'March20 - Feb21')))
...
## number_of_treatments VS covid time
