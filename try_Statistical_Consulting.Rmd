...
## covid time VS atopic diseases
```{r, warning=F}
atopic_longer_count <- df %>% select(c(exploreatory_variables, covid_time)) %>% group_by(covid_time) %>% summarise_if(is.numeric, sum) %>% pivot_longer(cols  =exploreatory_variables, names_to = "chronic_disease", values_to = "count") 
