...
ftable(xxx$covid_time, xxx$is_atopic_first_year) / rowSums(ftable(xxx$covid_time, xxx$is_atopic_first_year))
# ftable(xxx$covid_time, xxx$is_atopic_first_year)/ #166
ftable(xxx$covid_time, xxx$is_atopic_first_year)

ftable(covid_time, is_atopic_first_year) / rowSums(ftable(covid_time, is_atopic_first_year))
...
ggplot(df %>% filter(number_of_ABX_first_year >0 ), aes(x=covid_time, fill=as.factor(number_of_ABX_first_year))) + geom_bar(position = "fill") + 
  labs( title = 'covid time VS number_of_ABX_first_year', x = 'covid time group', y = 'count', fill = 'number of antib...') + theme_bw()
df %>% filter(number_of_ABX_first_year >0 )%>% group_by(covid_time) %>% summarise(avg_of_ABX_first_year = mean(number_of_ABX_first_year))


ggplot(df %>% filter(number_inhalation >0 ), aes(x=covid_time, fill=as.factor(number_inhalation))) + geom_bar(position = "fill") + 
  labs( title = 'covid time VS number_inhalation', x = 'covid time group', y = 'count', fill = 'number of antib...') + theme_bw()
df %>% filter(number_inhalation >0 )%>% group_by(covid_time) %>% summarise(avg_number_inhalation = mean(number_inhalation))


ggplot(df %>% filter(number_steroid_treat >0 ), aes(x=covid_time, fill=as.factor(number_steroid_treat))) + geom_bar(position = "fill") + 
  labs( title = 'covid time VS steroid_treat', x = 'covid time group', y = 'count', fill = 'number of antib...') + theme_bw()
