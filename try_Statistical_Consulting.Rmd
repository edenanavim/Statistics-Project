...
```{r}
plot_df_1 <- df %>% select(covid_time, AD_first_year, FA_first_year, ABX_fisrt_year,food_allergy_24_month, inhalation_24_month, AD_24_month) %>% group_by(covid_time) %>% summarise(AD_first_year = sum(AD_first_year), FA_first_year = sum(FA_first_year), ABX_fisrt_year = sum(ABX_fisrt_year) ,food_allergy_24_month = sum(food_allergy_24_month), inhalation_24_month = sum(inhalation_24_month), AD_24_month = sum(AD_24_month)) %>% gather(chronic_disease, count, AD_first_year:AD_24_month, factor_key=TRUE)

ggplot(plot_df_1,aes(x=chronic_disease, y= count, fill = as.factor(covid_time))) +
    geom_col(position = "dodge")+ theme(axis.text.x = element_text(angle = 45, vjust = 0.7)) + scale_fill_discrete(name = "Covid Group")+ xlab("Chronic Disease") + ylab("Cases")
